...
## 2026-08-09 — Phase 5 完成与 Phase 6 启动

### 目标

- 修正仍停留在“等待生产凭据”的现行文档，正式结束 Phase 5 并进入 LiveBoard 后端接入阶段。

### 实现

- README、总体计划、架构和动态进度页统一更新为 Phase 0–5 完成、Phase 6 进行中。
- Phase 5 参考标记为完成；正式 client 的 OIDC/Directory `picture` 与 `user.profile.changed` webhook 归入 Phase 6 接入验收。
- 真实成员邀请和中国大陆/境外性能测量保留为非阻塞运营验收，不再阻塞阶段切换。
- Phase 4 参考修正为资料事件已由 Phase 5 实现，不再保留未来时表述。

### 关键决定或问题

- HFLive Auth 核心平台已经生产上线，但 LiveBoard 尚未接入；文档分别表述平台生产状态与首个接入方状态，避免将两者混为一谈。
- Phase 6 先进行 LiveBoard 现有认证、用户、session、数据库和部署结构的只读审查及设计冻结，再修改实现。

### 验证

- 全仓搜索并消除现行 README、总体计划、架构、动态进度和 Phase 4/5 参考中的过时阶段结论；历史开发日志保持原有时点记录不变。
- `git diff --check` 通过。
...
## 2026-08-11 — 会话感知首页、个人资料与邀请有效期

### 目标

- 让 HFLive Auth 用户界面正确识别已有会话，统一认证服务命名，并把单一头像工具升级为可信的账号资料页。
- 允许管理员按场景控制邀请链接有效期，并明确未接受用户名的释放规则。

### 实现

- 首页按服务端会话显示当前账号摘要；已登录时移除登录按钮，改为资料管理及按权限显示管理后台。登录页在已有会话时直接继续同源 callback/OIDC authorize，请求外部 callback 时回到安全的 `/profile`。
- 用户界面、事务邮件与授权说明中，指向 `auth.hsfz.live` 的服务统一称为 `HFLive Auth` 或“Live 统一认证系统”；协议标识、API 路径和事件 Header 保持兼容。
- `/profile` 改为“个人资料”结构，分层展示账号头像、显示名、用户名、邮箱验证、账号类型和加入时间；只把当前可修改的头像标为“可编辑”，不伪造其他资料编辑入口。
- 首页品牌使用 `icon.svg` 几何标志，并在深色页面以浅色 mask 渲染；favicon 自身根据浏览器配色选择深/浅图形。
- 邀请表单和 API 支持 `2h | 1d | 7d | 30d`；邮件、顶部成功通知和审计 metadata 使用服务端确认的有效期。创建邀请前原子标记所有到期 pending 邀请为 `EXPIRED`，释放其邮箱和大小写不敏感用户名。

### 验证

- `pnpm validate`：13 个测试文件、44 项测试通过，4 个数据库/环境专项文件按条件跳过。
- `pnpm test:phase3`：7 项通过，新增到期用户名释放和重新预留验证。
- `pnpm test:phase5`：2 项头像对象存储、规范化和资料事件测试通过。
- 使用一次性构建占位 secret 的 `STATIC_ASSET_PROVIDER=vercel pnpm build` 通过；外部静态上传按非 Vercel Production 规则跳过。
- Edge 本地真实浏览器确认：已有会话访问 OIDC 登录入口直接进入 consent 且无密码框；首页已登录时无登录按钮，未登录时两个操作同为 46px 且顶部/底部对齐；个人资料展示完整账号字段，页面和邀请表单在桌面宽度均无横向溢出。

### 遗留事项

- 合并后由用户从既有 `cyx2007/LiveSSO` 来源触发 Vercel 部署，再复验正式域名会话复用、资料页和邀请邮件。

## 2026-08-11 — 资料编辑、邀请故障诊断与资源显示修正

### 目标

- 修复正式环境邀请失败时误报占用、页头品牌图标受静态域名影响，以及资料页缺少显示名编辑和外部返回语义的问题。

### 实现

- 邀请 API 将已有账号、待处理预留、邮件投递失败和数据库故障拆分为稳定错误代码；邀请与审计改为同一事务，投递失败会撤销邀请。管理页面显示对应且可操作的错误提示。
- Vercel Production 构建改为先执行 `prisma migrate deploy`，成功后再运行 Next.js 构建；Preview 不执行正式数据库迁移。由此避免代码已引用新字段但生产 schema 尚未迁移的状态。
- 页头品牌标记改为内联 SVG，不再由 EdgeOne CSS 以根路径请求 `icon.svg`；浏览器 favicon 继续由 Next.js metadata route 提供。
- 新增本人修改显示名的 `PATCH /api/profile`，更新事务写入审计并为订阅 client 建立 `user.profile.changed` outbox；资料页即时更新头像占位符和标题。
- 外部应用进入资料页时，页头显示“完成并返回”和经审批 client 解析出的产品名；正式 LiveBoard 域名展示为 `LiveBoard`。目标仍限制为已审批、启用 client 的 HTTPS redirect origin，直接访问时保留“返回首页”。

### 验证

- `pnpm validate`：15 个测试文件、50 项测试通过，4 个文件按环境跳过。
- `pnpm test:phase3`：7 项邀请、登录和风险验证集成测试通过。
- `pnpm test:phase5`：3 项头像、显示名与资料事件集成测试通过。
- 生产形态 `pnpm vercel:build`：迁移检查先成功完成，随后 Next.js 生产构建和 Vercel 静态资源路径完成。
- 真实浏览器 1280×720 验证品牌标记可见、外部资料页显示“完成并返回 LiveBoard”、显示名保存后页面即时更新且无横向溢出；直接访问资料页只显示“返回首页”。

## 2026-08-11 — 邀请成功反馈误报修正

- 修复管理员邀请已经创建且邮件已经发出后，页面却显示“无法连接邀请服务”的问题。
