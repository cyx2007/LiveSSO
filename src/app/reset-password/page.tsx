...
export default async function ResetPasswordPage({ searchParams }: { searchParams: Promise<{ token?: string; error?: string }> }) {
  ...
  return <main className="auth-main"><section className="panel auth-card">
    <p className="eyebrow">Recovery</p><h1 className="auth-title">设置新密码</h1>
    <p className="auth-copy">新密码至少 12 个字符。完成后现有会话会被撤销。</p>
    {token && !error ? <ResetPasswordForm token={token} /> : <p className="form-error" role="alert">重置链接无效或已过期，请重新发起找回密码。</p>}
  </section></main>;
}
