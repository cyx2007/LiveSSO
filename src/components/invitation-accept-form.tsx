...
export function InvitationAcceptForm({ token }: { token: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/invitations/accept", {
      ...
      body: JSON.stringify({ token, username: data.get("username"), name: data.get("name"), password: data.get("password") }),
    });
    setPending(false);
  ...
  return (
    <form onSubmit={submit}>
      ...
      <div className="field"><label htmlFor="username">用户名</label><input id="username" name="username" minLength={3} maxLength={32} pattern="[A-Za-z0-9_]+" autoCapitalize="none" required /></div>
